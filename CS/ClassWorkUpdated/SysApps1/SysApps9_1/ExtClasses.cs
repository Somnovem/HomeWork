...
namespace SysApps9_1
{
    internal class ExtClasses
    {
        ...
        public static extern IntPtr GetModuleHandle([MarshalAs(UnmanagedType.LPWStr)] string lpModuleName); //возвращает дескриптор модуля по его имени
        ...
        public static IntPtr SetHook(HookProc hookProc, HookType hookType)
        {
            using (Process process = Process.GetCurrentProcess())
            {
                using (ProcessModule module = process.MainModule)
                {
                    ...
                    {
                        return SetWindowsHookEx(hookType, hookProc, handle, 0);//тип,фильтрующяя функция, дескриптор модуля, айди потока который будет отслеживаться(0 - следить за всеми)
                    }
                    else
                        return IntPtr.Zero;
                }
